...
import type { IntercomSystem } from '../systems/IntercomSystem';
...
/** Sim world is 2× the old playfield so the AI ship has room to dodge. */
const W = 640;
const H = 400;
...
export class AsteroidsSim {
  ...
  constructor(power: PowerSystem, state: GameState, intercom: IntercomSystem) {
    this.power = power;
    this.state = state;
  ...
  seedWave(wave: number): void {
    ...
    const rocks = 3 + wave;
    for (let i = 0; i < rocks; i++) this.spawnAsteroid(2);
  ...
  update(dt: number): void {
    ...
    if (this.waveTimer > 45) {
      this.waveTimer = 0;
      this.state.wave += 1;
      bus.emit('waveAdvanced', { wave: this.state.wave });
    ...
    if (this.spawnTimer <= 0) {
      ...
      if (Math.random() < 0.55) this.spawnAsteroid(PhaserRandSize());
  ...
  private drainFuel(dt: number): void {
    const drain = this.power.fuelDrainPerSec() * dt;
    this.state.fuel = Math.max(0, this.state.fuel - drain);
    if (this.state.fuel <= 0 && this.state.pips.engines > 0) {
      // Engines starve — soft fail after brief grace handled in scene
    }
  }
  ...
  private ai(dt: number): void {
    ...
    if (best) {
      this.targetAngle = Math.atan2(best.y - this.ship.y, best.x - this.ship.x);
    } else {
      this.targetAngle += dt * 0.4;
    }

    let da = wrapAngle(this.targetAngle - this.ship.angle);
    this.ship.angle += PhaserClamp(da, -3 * dt, 3 * dt);
  ...
  private fire(weapon: 'cannon' | 'mg'): void {
    if (weapon === 'cannon') {
      ...
      // Consume ammo gradually
      if (Math.random() < 0.35) this.state.ammoBoxes -= 1;
    }
    bus.emit('weaponFired', { weapon });
  ...
  private integrate(dt: number): void {
    ...
    for (const b of this.bodies) {
      ...
      // Bounce rocks/enemies at edges (no wrap)
      if (b.kind !== 'bullet') {
  ...
  private collide(): void {
    const shieldChance = this.power.shieldMult() * (1 - this.intercom.ignoredPenalty('shields') * 0.5);
  ...
  private destroyBody(b: SimBody): void {
    if (b.kind === 'asteroid') {
      bus.emit('asteroidMined', { amount: b.size ?? 1 });
      this.state.score += 10 * (b.size ?? 1);
      bus.emit('scoreChanged', { score: this.state.score });
      this.splitOrRemove(b);
    } else if (b.kind === 'enemy') {
      bus.emit('enemyDestroyed', { score: 50 });
      this.state.score += 50;
      bus.emit('scoreChanged', { score: this.state.score });
      this.removeBody(b);
    }
...
function PhaserClamp(v: number, a: number, b: number): number {
  return clamp(v, a, b);
}
function PhaserRandSize(): number {
  return 1 + Math.floor(Math.random() * 3);
}

export const ASTEROIDS_W = W;
export const ASTEROIDS_H = H;
